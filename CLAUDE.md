...
- **Targets:** `index.html` → `Home.md`, `courses.html` → `Courses.md`, `projects.html` → `Projects.md`, `trips.html` → `Trips.md`, `lifts.html` → `Lifts.md`. Skip `books.html`, skip `gates.html` (a game — its text is UI chrome and tutorial copy, not writing), and skip any page with placeholder/empty content (e.g. `experience.html` while WIP). If a skipped page has gained real content, export it too (capitalized filename matching page title).
