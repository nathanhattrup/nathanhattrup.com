...
**Note:** Projects and Experience pages are still WIP — structure exists but content is incomplete. Lifts and Gates are linked from every sidebar (Gates listed under Lifts) and in the sitemap.
...
- `make_gates_data.py` — Generates `assets/gates-data.js` (`const GATES_DATA`, 200 puzzles, ~22 KB). Fully deterministic (frozen `SHUFFLE_SEED`); `EPOCH = "2026-08-09"` is the launch date and must NEVER change after launch — it remaps every past date's puzzle and corrupts stored histories/share strings. Output is committed; rerun only if the ruleset changes. Tests: `python3 test_gates_data.py`-style runner or pytest (`test_gates_data.py`; invariants, self-consistency, byte-reproducibility)
- `assets/gates.js` — All Gates game logic in one classic-script IIFE (dating, storage `localStorage["gates.v4"]`, hand-rolled circuit SVG, tri-state table, lives, share, archive, tutorial modal). Key rules: C/D scratch columns are NEVER validated or marked; Q feedback is one bit; duplicate detection compares Q only; archive plays never touch the streak; player-facing number is the unwrapped day count, puzzle id stays internal. Pre-launch the page shows a banner + preview button (`?preview=1` autostarts; preview records nothing)
